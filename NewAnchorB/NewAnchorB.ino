/**
 * @file AnchorB.ino
 * @brief Decawave DW1000Ng를 사용한 앵커 B의 거리 측정 시스템.
...
// MQTT 토픽 설정
const char* mqtt_topic = "myRange/B";
...
void setup() {
  ...
  Serial.println(F("### DW1000Ng-arduino-ranging-anchorB ###"));
  // DW1000 모듈 초기화
...
/**
 * @brief 주 실행 루프
 *        MQTT 상태를 확인하고, 앵커 B에서 타임 슬롯을 기반으로 거리 측정 작업을 수행합니다.
 */
void loop() {
  if (!client.connected()) {
    reconnect();
  ...
  // 현재 시간의 초에 따라 Anchor B가 활성화됨
  if (currentSlot == ANCHOR_B_SLOT) {
